...
int main()
{
    ifstream file("testecc.txt");
    file>>a>>b>>p;
    file.close();
    ofstream wfile;
    wfile.open("testecc.txt",ios::app);
    wfile<<endl;
    int i,j;
    int arr1[p],arr2[p];
    ...
    wfile<<"The Affine points:"<<endl;
    for(i=0;i<arr3.size();i++)
    {
        wfile<<"("<<arr3[i]<<","<<arr4[i]<<") ";
    }
    wfile<<endl;
    wfile<<"The generator point:"<<endl;
    wfile<<"("<<arr3[4]<<","<<arr4[4]<<")"<<endl;
    wfile<<"subgroup of generator point:"<<endl;
    int sub1[100],sub2[100],t,temp,temp1,temp2;
    sub1[0]=arr3[4],sub2[0]=arr4[4];
    for(i=1;;i++)
    {
        if(i%2==1)
        {
    ...
    for(i=0;;i++)
    {
        wfile<<(i+1)<<"G=("<<sub1[i]<<","<<sub2[i]<<") ";
        if(sub1[i]==0 && sub2[i]==0)
            break;
    }
    wfile<<endl;
    wfile<<"Value of n: "<<(i+1)<<endl;
    wfile.close();
    return 0;
}
